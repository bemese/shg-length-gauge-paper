...
\cite{levinePRL89, levinePRL91} This allows us to ``open'' the DFT-LDA gap to 
...
Refs.~\onlinecite{mendozaPRL98,arzatePRB01,mendozaPRB01,mejiaPRB02} followed 
...
intraband, $1\omega$ and $2\omega$ terms. Perhaps of more relevance to this 
work, Mej\'ia \textit{et al}.\cite{mejiaRMF04} developed a layer-by-layer 
...
of the valence electrons with the core electrons. As it will be discussed 
in this article, dependence on the position vector introduces corrections to
the velocity operator of the electron that have to be included with care in 
...
pseudopotentials. Therefore, the goal of this paper is to derive new 
expressions within the length gauge for the SSH susceptibility tensor 
$\chi^{\textrm{a}\textrm{b}\textrm{c}}(-2\omega;\omega,\omega)$ 
...
$2 \times 1$ reconstruction. We compare results from before and after adding 
...
band gap of this surface, and $E_g=1.035$ eV is the scissor shifted gap for
$\hbar\sigma=0.5$ eV.
These resonances
...
$\bfgchi(-2\go;\go,\go)$. The scissors correction shifts the spectrum to 
higher energies though the shifting is not rigid and mixes the 
$1\omega$ and $2\omega$ resonances, the cut function allows us
